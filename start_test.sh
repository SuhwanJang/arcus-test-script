...
function memtier_setting(){ 
    ...
    read -r -p "Test case (multiple select: 1, 2, 3) 
     ...
     2) Retrieve Data in uniform Random
     3) Retrieve Data in Gaussian distribution (longtail)
     4) Insertion & Retrieve Data - Random (1:9)
     5) Insertion & Retrieve Data - Random (3:7)
     6) Insertion & Retrieve Data - Random (5:5)
     7) Insertion & Retrieve Data - longtail (1:9)
     8) Insertion & Retrieve Data - longtail (3:7)
     9) Insertion & Retrieve Data - longtail (5:5)
     >> " client_mode
...
function client_insertion(){
    ...
    do
        ...
        if [[ "${#MPID}" == "0"* ]];then echo "  :Insertion opertation complete"; sleep 6; break; fi
    done
}
function client_record_run(){
    ...
    do
        sleep 10
        MPID=$(ssh -T persistence@211.249.63.38 -p $remote pgrep memtier)
        if [[ "${#MPID}" == "0"* ]]; then sleep 6; break; fi
    done
}
# Record log
function record_log(){ 
